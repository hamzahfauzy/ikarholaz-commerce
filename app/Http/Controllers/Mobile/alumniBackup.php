...
Saat ini status masih PENDING hingga diverifikasi petugas. Ketik INFO NRA untuk mengetahui status pendaftaran anggota IKARHOLAZ.";
...
$pesan = 
"Registrasi GAGAL. Nomer WA sudah digunakan pendaftaran sebelumnya. Ketik STATUS untuk mengetahui data terkait nomer WA yang digunakan.";
    ...
    function getNra(Request $request)
    {
        ...
        // USAHA DAN BISNIS
        return json_encode($dataUsaha);
        $status = $alumni->approval_status == "approved" ? "APPROVED" : "PENDING";
        $photo = $alumni->profile_pic == NULL ? "_belum disi_" : url('/storage/public')."/".$alumni->profile_pic;
        $ttl = $alumni->place_of_birth == NULL ? "_belum disi_" : $alumni->place_of_birth . "/" . $alumni->date_of_birth;
