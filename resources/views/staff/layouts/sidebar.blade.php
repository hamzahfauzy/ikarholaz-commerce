    ...
    <div class="sidebar-inner slimscrollleft">
        ...
        <div id="sidebar-menu">
            <ul>
                ...
                @if($menu['childs'])
                <li class="has_sub">
                    <a href="javascript:void(0);" class="waves-effect"><i class="{{$menu['icon']}}"></i><span> {{__($menu['label'])}} </span> <span class="menu-arrow"></span></a>
                    <ul class="list-unstyled">
                        @foreach($menu['childs'] as $child)
                        <li><a href="{{Route::has($child['route'])?route($child['route']):''}}">{{__($child['label'])}}</a></li>
                        @endforeach
                    </ul>
                </li>
                @else
                <li>
                    <a href="{{Route::has($menu['route'])?route($menu['route']):''}}" class="waves-effect"><i class="{{$menu['icon']}}"></i><span> {{__($menu['label'])}} </span></a>
                </li>
                @endif
                @endforeach
